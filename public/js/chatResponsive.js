...
const abrirChat = async (e) => {
    ...
    const res = await req.json();
    const _chat = res.chat;
    const chatDiv = await createChatHTML(id);
    let otherParticipant = _chat.participants.filter(participant => participant.username !== userid)
    chatDiv.children[1].innerText = otherParticipant[0].username
    //Asignamos al name container el nombre del otro participante del chat
    chat.appendChild(chatDiv);
    chat.classList.remove('hide')
    chatTab.parentNode.classList.add('hide')
}
...
const crearChat = async (e) => {
    ...
    const res = await req.json();
    const chatid = res.id;
    const _chat  =await createChatHTML(chatid)
    _chat.children[1].innerText = targetUser
    chat.appendChild(_chat);
    chat.classList.remove('hide')
    chatTab.parentNode.classList.add('hide')
}
...
const createChatHTML = async (chatid) => {
    ...
    const returnToChats = document.createElement('div');
    ...
    returnToChats.classList.add('return-to-chats')
    ...
    returnToChats.addEventListener('click', killChat)
    ...
    chatContainer.appendChild(returnToChats);
...
const killChat = (e) => {
    ...
    const chatDiv = e.currentTarget.parentElement.parentElement;
    chatDiv.classList.add('hide')
    chatDiv.innerHTML = '';
    chatTab.parentNode.classList.remove('hide');
}
